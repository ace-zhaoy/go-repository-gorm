...
func RunWithMySQLInContainer() (endpoint string, teardown func()) {
...
func TestMain(m *testing.M) {
	endpoint, teardown := RunWithMySQLInContainer()
	defer teardown()
...
type User1 struct {
	ID        int64                 `json:"id" gorm:"COLUMN:gorm_id"`
	DeletedAt soft_delete.DeletedAt `json:"deleted_at" gorm:"COLUMN:gorm_deleted_at"`
}
...
type User2 struct {
	...
	DeletedAt soft_delete.DeletedAt `json:"json_deleted_at"`
}
...
func TestCrudRepository_IDField(t *testing.T) {
	var db *gorm.DB
	repository1 := NewCrudRepository[int64, *User1](db)
	assert.Equal(t, repository1.IDField(), "gorm_id")
	repository2 := NewCrudRepository[int64, *User2](db)
...
func TestCrudRepository_SoftDeleteField(t *testing.T) {
	var db *gorm.DB
	repository1 := NewCrudRepository[int64, *User1](db)
	assert.Equal(t, repository1.SoftDeleteField(), "gorm_deleted_at")
	repository2 := NewCrudRepository[int64, *User2](db)
	assert.Equal(t, repository2.SoftDeleteField(), "json_deleted_at")
	repository3 := NewCrudRepository[int64, *User3](db)
...
var userSql = "CREATE TABLE `user` (" +
	"`id` bigint UNSIGNED NOT NULL AUTO_INCREMENT," +
	"`name` varchar(32) NOT NULL COMMENT '姓名'," +
	"`created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间'," +
	"`updated_at` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'," +
	"`deleted_at` int UNSIGNED NOT NULL DEFAULT '0' COMMENT '删除写入时间戳'," +
	"PRIMARY KEY (`id`)" +
	") ENGINE = InnoDB CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT '用户表';"

func initUserTable() (db *gorm.DB, teardown func()) {
	defer errors.Recover(func(e error) { log.Fatalf("initUserTable err: %+v", e) })
	dsn := fmt.Sprintf("root:%s@tcp(%s)/%s?charset=utf8mb4&parseTime=True&loc=Local", rootPassword, mysqlEndpoint, databaseName)
	db, err := gorm.Open(mysql.Open(dsn), &gorm.Config{
		TranslateError: true,
	})
	errors.Check(errors.Wrap(err, "failed to open db"))
	err = db.Exec(userSql).Error
	errors.Check(errors.Wrap(err, "failed to create table"))
	return db, func() {
		err = db.Exec("DROP TABLE `user`").Error
...
func TestCrudRepository_FindOne(t *testing.T) {
	...
	foundUser, err := userRepository.FindOne(context.Background(), map[string]any{"name": "test"})
	errors.Check(errors.Wrap(err, "failed to find user"))
	assert.Equal(t, foundUser.Name, user.Name)

	_, err = userRepository.FindOne(context.Background(), map[string]any{"name": "nonexistent"})
...
func TestCrudRepository_FindOne_WithOrder(t *testing.T) {
	...
	foundUser, err := userRepository.FindOne(context.Background(), map[string]any{"name": []string{"test1", "test2"}}, contract.Order{
...
func TestCrudRepository_FindByFilterWithSort(t *testing.T) {
	...
	// 按照 Name 字段升序排序
	collection, err := userRepository.FindByFilterWithSort(context.Background(), map[string]any{}, contract.Order{
		Key:   "name",
		Value: 1, // 升序
	})
